...
const LingkupMateri: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/Lingkupmateri/${id}`);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.patch(
        `http://localhost:5000/Lingkupmateri/${editData._id}`,
        {
